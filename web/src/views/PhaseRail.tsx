 ...
 * grouped into stages (phases that can run at the same time), in a strip that
...
function PhaseChip({
  ...
  const border =
    pill.status === "failed"
      ? "border-fail/40 bg-surface hover:bg-ground-2"
      : pill.status === "await"
        ? "border-await/40 bg-surface hover:bg-ground-2"
