...
- **The runtimes (`runtimes/claude.ts`, `codex.ts`, `opencode.ts`, `qwen.ts`)
  ...
  read-only KnowledgeContext file, the artifact and memory directories —
  reach the runtimes as one list (`harness/channels.ts`), and each runtime
  answers per channel whether its sandbox can reach it; the engine decides
  what that means from whether the launch depends on the channel
  (HARNESS.md §3a).
...
| Knowledge Ledger  | `argus/knowledge.json`                                                 | authoritative, append-only claims/evidence/justifications/consumptions/artifacts/deltas (v3); support, currency and impact derived per read                                                                                            |
| Knowledge deltas  | `argus/knowledge-deltas/<runId>/`                                      | per-run: the agent's proposal (`delta.json`) and Argus's staging record (`staged.json`); never canonical, committed only at phase acceptance                                                                                           |
| Knowledge context | `argus/invocations/<runId>/knowledge-context.json` + `invocation.json` | per-run, read-only: the exact claim revisions Argus supplied to the run (the projection) and, on the invocation record, their refs and sha256 — the authoritative "supplied" provenance; supplied/consumed comparison derived per read |
