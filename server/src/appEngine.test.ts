...
const config: ArgusConfig = {
  port: 7777, host: "127.0.0.1", token: null, allowedHosts: [], allowedOrigins: [],
  maxConcurrentRuns: 4, schedulerTickMs: 30000, webhookUrl: null,
};
...
const same = { host: "localhost:7777", origin: "http://localhost:7777", "content-type": "application/json" };
...
async function createPipeline(app: ReturnType<typeof appWith>["app"]) {
  const res = await app.request("/api/pipelines", {
    ...
    body: JSON.stringify({
      name: "p",
      trigger: null,
      phases: [{ id: "only", name: "Only", cwd: home, gated: false, steps: [{ name: "s", prompt: "go" }] }],
    }),
  });
...
test("real engine: preflight failure maps to 412 with reasons", async () => {
  ...
  const res = await app.request(`/api/pipelines/${def.id}/start`, { method: "POST", headers: same });
...
test("real engine: start → 202, then a bad-token signal → 403", async () => {
  const { app } = appWith();
  const def = await createPipeline(app);
  const start = await app.request(`/api/pipelines/${def.id}/start`, { method: "POST", headers: same });
...
test("real engine: abort → 200, then a second abort → 409", async () => {
  ...
  const first = await app.request(`/api/instances/${inst.id}/abort`, { method: "POST", headers: same });
  assert.equal(first.status, 200);
  const second = await app.request(`/api/instances/${inst.id}/abort`, { method: "POST", headers: same });
...
test("real engine: overlap=skip refuses a second concurrent start with 409", async () => {
  const { app } = appWith();
  const def = await createPipeline(app);
  const first = await app.request(`/api/pipelines/${def.id}/start`, { method: "POST", headers: same });
  assert.equal(first.status, 202);
  const second = await app.request(`/api/pipelines/${def.id}/start`, { method: "POST", headers: same });
  assert.equal(second.status, 409);
});
